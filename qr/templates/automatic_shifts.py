...
class OrganisationShifts:
    def __init__(self, organisation_id, shift_id):
        ...
        self.q = PQueue(self.no_of_employees_needed)
        self.shift_emp_list = []
    ...
    # Least ratio first
    def orderByPctContractedHours(self, shift_employees):
        sorted_employees = sorted(shift_employees, key=operator.attrgetter("ratio"))
        return sorted_employees
    ...
    def getStaffsForShift(self):
        ...
        shift_employees = self.orderByPctContractedHours(employees)
...
class Employee:
    def __init__(self, id, branch_name, contracted_hours, worked_hours, ratio):
        ...
        # self.pct = (float(self.worked_hours) / self.contracted_hours) * 100
        self.ratio = ratio
