...
workerList = ['EE01', 'EE02', 'EE03', 'EE04', 'EE06', 'EE07', 'EE08', 'EE09', 'EE10']

##Define shift requirment
...
##Assume everyone is availble
availability = pandas.DataFrame(numpy.ones((len(workerList), len(shiftList))), index=workerList, columns=shiftList)
...
sol = pandas.DataFrame(data={'Solution': model.X}, index=model.VarName)
sol = sol.iloc[0:len(x)]

dashboard = pandas.DataFrame(index=workerList, columns=shiftList)
for w in workerList:
    for s in shiftList:
